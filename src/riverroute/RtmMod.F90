module RtmMod
   ...
   ! !DESCRIPTION:
   ! River routing model
   ! Modification: on top of irrigation volume, domestic/ livestock/ thermoelectric/
   ! manufacturing and mining water usage are taken
   !
   ! !USES:
   !
   ! !ARGUMENTS:
   ...
   ! parameters used in negative runoff partitioning algorithm
       ...
       real(r8) :: elec_volume                ! volume of thermoelectric usage demand during time step [m3]
       real(r8) :: liv_volume                ! volume of livestock usage demand during time step [m3]
       real(r8) :: mfc_volume                ! volume of manufacturing  demand during time step [m3]
       real(r8) :: min_volume                ! volume of mining demand during time step [m3]
   ...
   !    endif
       ...
       do nr = rtmCTL%begr,rtmCTL%endr
             ...
             ! calculate volume of domestic flux during timestep
             dom_volume = -rtmCTL%qdom_withd(nr) * coupling_period
             ! calculate volume of livestock flux during timestep
             liv_volume = -rtmCTL%qliv_withd(nr) * coupling_period
             ! calculate volume of thermoelectric flux during timestep
             elec_volume = -rtmCTL%qelec_withd(nr) * coupling_period
             ! calculate volume of manufacturing flux during timestep
             mfc_volume = -rtmCTL%qmfc_withd(nr) * coupling_period
             ! calculate volume of mining flux during timestep
             min_volume = -rtmCTL%qmin_withd(nr) * coupling_period
   ...
   !       if (rtmCTL%mask(nr) == 1) then
            ...
            ! actual irrigation/domestic/thermoelectric/livestick/mining/manufacturing rate [m3/s]
   ...
   !    if (budget_check) then
          ...
          do nt = 1,nt_rtm
             ...
             budget_input  = (budget_terms(13,nt) + budget_terms(14,nt) + &
                              ...
                              ! Modification: add dom/elec/liv/mfc/min
